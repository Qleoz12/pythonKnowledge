...
if __name__ == '__main__':
    ...
    with open('D:\ETL\llamadas-123-mes-de-mayo-2020.csv', 'r') as file:
        ...
        for row in reader:
            ...
            # exists = sesion.query(dim_localidad).filter_by(codigo=dim_localidad_e.codigo, localidad = dim_localidad_e.localidad).first() is not None
            # if not exists:
            ...
            # exists = sesion.query(dim_incidente).filter_by(incidente=dim_incidente_e.incidente,prioridad=dim_incidente_e.prioridad).first() is not None
            # if not exists:
            #     sesion.add(dim_incidente_e)
            #     sesion.commit()

            dim_incidente_c_id = sesion.query(dim_incidente.id).filter(dim_incidente.incidente == row['TIPO_INCIDENTE']).first()
            dim_red_c_id = sesion.query(dim_red.id).filter(dim_red.red == row['RED']).first()
            dim_localidad_c_id = sesion.query(dim_localidad.id).filter_by(codigo=dim_localidad_e.codigo, localidad = dim_localidad_e.localidad).first()
