...
def extraccion():
    ...
    for i, tweet in enumerate(sntwitter.TwitterSearchScraper('etl since:2020-08-01 until:2021-09-11').get_items()):
        if i > maxTweets:
            break
        tweets_list2.append(
            [tweet.date, tweet.url, tweet.lang, tweet.retweetCount, tweet.id, tweet.content, tweet.user.username])

    # Creating a dataframe from the tweets list above
    tweets_df2 = pd.DataFrame(tweets_list2,
                              columns=['Datetime', 'url of tweet', 'lenguaje', 're-twittes', 'Tweet Id', 'Text',
                                       'Username'])
